...
def main(dataset,
    ...
    color_maps = ['magma', 'plasma', 'BLUE', 'magma']#'thermal', 'haline', 'viridis']
    for fld, cmap in yt.parallel_objects(zip(fields, color_maps)):
        ...
        if fld == 'density':
            ...
            slc_bv = yt.SlicePlot(ds, slc_norm, fld,
                                   north_vector = norm_vector,
                                   center=center, width=length,
                                   data_source=sph)
            slc_bv.set_axes_unit('kpc')
            slc_bv.set_cmap(field=fld, cmap=cmap)
            slc_bv.set_background_color(fld)
