...
def create_axes_proj(ds,
                    ...
                    output_filename_head = './'):
    try:
        os.makedirs(output_filename_head+"annotated_xz_plots")
    except OSError as e:
        if e.errno != errno.EEXIST:
            raise

    proj_filename = (output_filename_head+'annotated_xz_plots/annotated_xz_plot_%s_%s_%s.png') % (ion_p_name(ion),ds,axis)
...
def combine_xz_plots(filenames,
    ...
    if center is None:
        center = [0.5, 0.5, 0.5]
    print("I got here, before making folder")
    try:
        print("Trying to make folder")
        os.makedirs(output_filename_head+"annotated_combined_xz_plots")
    except OSError as e:
        print("ERROR")
        if e.errno != errno.EEXIST:
            raise
        ...
        for ion in ion_species:
            ...
            if ylim1x is None and ylim2x is None:
                if ion == 'O VI':
                    ...
                    ylim1 = ylim1x
                    ylim2 = ylim2x
            print(ion,ds,"= ylim1x, ylim2x is:",ylim1x,"and",ylim2x)
            print(ion,ds,"= ylim1,ylim2 is:",ylim1,ylim2)
            xz_plot_filename = (output_filename_head+'annotated_combined_xz_plots/annotated_combined_xz_plot_%s_%s.png') % (ion_p_name(ion),ds)
            if not os.path.exists(xz_plot_filename):  
                ...
                # string everything together in a single image
                list_im = [output_filename_head+'annotated_xz_plots/annotated_xz_plot_%s_%s_0.png' % (ion_p_name(ion),ds), 
                           output_filename_head+'annotated_xz_plots/annotated_xz_plot_%s_%s_2.png' % (ion_p_name(ion),ds)]
                print("break 1")
                try:
                    imgs    = [ PIL.Image.open(i) for i in list_im ]
                except:
                    print("come on")
                print("error with PIL open???")
                max_shape = sorted( [(i.size[0], i.size ) for i in imgs])[1][1]
                #print([(i.size[0], i.size ) for i in imgs])
                # print("Max shape is",max_shape,"\n")
                print("Tried to combine xz_plots for %s,%s" % (ion,ds))
                # for a vertical stacking it is simple: use vstack
                #print((np.asarray( i.resize(max_shape[0],i.size[1]*max_shape[0]/i.size[0]) ) for i in imgs ) )
                temp_list = []
                another_list = []
                ...
                if yt.is_root():
                    print("Written:",ds)
                    file = open(output_filename_head+"annotated_filenames_xz_plots.fn",'a+')
                    file.write(filenames[:-13]+"%s/%s -- %s\n" % (ds,ds,ion))
                    file.close()
